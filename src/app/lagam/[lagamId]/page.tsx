...
export default function LagamDetailPage() {
  ...
  // Lagam state
  const [lagamData, setLagamData] = useState<Lagam | null>(null);
  const [productInfo, setProductInfo] = useState<Partial<ProductInfo>>({ sizes: [{ size: '', quantity: 0 }] });
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const [teamsRes, usersRes, catalogRes, lagamRes] = await Promise.all([
          fetch("/api/teams"),
          fetch("/api/users"),
          fetch("/api/catalog"),
          fetch(`/api/lagam/${lagamId}`),
        ]);
        if (!teamsRes.ok || !usersRes.ok || !catalogRes.ok || !lagamRes.ok) {
          throw new Error("Failed to fetch initial data");
        }
        const teamsData = await teamsRes.json();
        const usersData = await usersRes.json();
        const catalogData = await catalogRes.json();
        const lagamDataRes: Lagam = await lagamRes.json();

        setTeams(teamsData);
        setUsers(usersData);
        setCatalog(catalogData);
        setLagamData(lagamDataRes);

        // Populate form with existing Lagam data
        setProductInfo(lagamDataRes.productInfo);
        setTeamInfo(lagamDataRes.teamInfo);
        setProductionBlueprint(lagamDataRes.productionBlueprint);
  ...
  const handleTeamSelect = (teamId: string) => {
    ...
    if (team) {
      const isDifferentTeam = teamInfo.assignedTeamId !== teamId;
      
      // Count only operators (users with role "Operator")
      const operatorCount = users.filter(u => 
        team.memberIds.includes(u.id) && u.role === 'Operator'
      ).length;
      
      setTeamInfo({
        assignedTeamId: team.id,
        assignedTeamName: team.name,
        teamMemberCount: operatorCount,
      });
      if (isDifferentTeam) {
          // Reset blueprint if team changes, as assignments will be invalid
          setProductionBlueprint(productionBlueprint.map(bp => ({ ...bp, assignedOperators: [] })));
          toast({ title: "Team changed", description: "Operator assignments have been cleared." });
      }
    }
  };
  ...
  const handleUpdateLagam = async () => {
    ...
    try {
        const response = await fetch(`/api/lagam/${lagamId}`, {
            method: 'PUT',
        ...
        if (!response.ok) {
            throw new Error('Failed to update Lagam');
        }

        toast({ title: 'Lagam updated successfully!' });
        router.push('/lagam');
        ...
        toast({
            variant: "destructive",
            title: "Error",
            description: "Could not update the Lagam. Please try again.",
        });
    }
  };
  ...
  if (loading) return <div>Loading Lagam details...</div>;

  return (
    <div className="flex flex-col gap-8">
      <PageHeader
        title={`Edit Lagam: ${productInfo.productName || ''}`}
        description={`ID: ${lagamId}`}
      />
                    ...
                    <div className="space-y-2">
                     ...
                     <div>
                        ...
                        <Input 
                          ...
                          value={productInfo.totalStandardTime?.replace(' minutes', '') || ''} 
                ...
                <div className="space-y-4">
                     ...
                     {teamInfo.assignedTeamId && (
                         <Alert>
                            ...
                            <AlertDescription>
                                Operator Count: {teamInfo.teamMemberCount}
                            </AlertDescription>
                        </Alert>
                     )}
                </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-6">
            ...
            <div className="flex justify-between items-center">
              ...
              <Button onClick={handleUpdateLagam} disabled={productionBlueprint.length === 0}>
                <Save className="mr-2 h-4 w-4" /> Update Lagam
              </Button>
            </div>
          </CardContent>
        </Card>
    ...
    const filteredCatalog = useMemo(() => {
        ...
        return catalog
            .map(section => {
                ...
                const sectionMatches = section.seccion.toLowerCase().includes(lowerSearch);
                
                // Filter operations that match
                const matchingOps = section.operaciones.filter(op =>
                    op.descripcion.toLowerCase().includes(lowerSearch) ||
                    op.maquina.toLowerCase().includes(lowerSearch)
                );
                
                // Include section if name matches OR it has matching operations
                if (sectionMatches || matchingOps.length > 0) {
                    return {
                        ...section,
                        // Show all operations if section name matches, otherwise only matching ones
                        operaciones: sectionMatches ? section.operaciones : matchingOps
                    };
                }
        ...
        <Dialog open={isOpen} onOpenChange={(open) => !open && close()}>
            <DialogContent className="max-w-3xl">
                ...
                <div className="relative mb-4">
                    <Input
                        type="text"
                        placeholder="Search sections and operations..."
                        value={searchTerm}
